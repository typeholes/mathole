...
export const initialState = {
    ...
    functionDefMap: { 
        ...
        zigZag: FunctionDef.newFunctionDef('zigZag', false, ['x'], '1-2 * acos((1- smoother) * sin(2 * pi * x))/pi'),
        squareWave: FunctionDef.newFunctionDef('squareWave', false, ['x'], '2 * atan( sin(2 * pi * x)/ smoother )/pi'),
        sawtooth: FunctionDef.newFunctionDef('sawtooth', false, ['x'], '(1+zigZag((2 * x - 1)/4) * squareWave(x/2))/2'),
        steps: FunctionDef.newFunctionDef('steps', false, ['x'], 'x - sawtooth(x)'),
        logSquares: FunctionDef.newFunctionDef('logSquares', false, ['a','b'], 'log(a^2+b^2)'),  // need a better name for this
        curvedSawtooth: FunctionDef.newFunctionDef('curvedSawtooth', false, ['x'], 'logSquares(x^smoother,sawtooth(x))'),
        calcMarketValue: FunctionDef.newFunctionDef('calcMarketValue', false, ['x'], 'curvedSawtooth(t)'),
    },
}
